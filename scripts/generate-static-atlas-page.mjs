...
function renderFieldOverview(graphData) {
  ...
  return `<section aria-labelledby="fields-heading">
    ...
    <div class="field-grid">${fieldIds.map((fieldId) => {
      ...
      return `<article class="field-card"><h3><a href="/${escapeHtml(field.path)}/">${escapeHtml(field.label)}</a></h3><p>${escapeHtml(field.description)}</p><ul class="domain-chips">${domains.map((domainId) => {
        const domain = graphData.domains[domainId];
        return `<li><span class="domain-swatch" style="--domain-color:${escapeHtml(domain.color)}" aria-hidden="true"></span>${escapeHtml(domain.label)}</li>`;
      }).join('')}</ul></article>`;
    }).join('')}</div>
  </section>`;
}
...
function renderConceptDirectory(graphData) {
  ...
  const sections = fieldIds.map((fieldId) => {
    ...
    const domainSections = domains.map((domainId) => {
      ...
      return `<section class="concept-domain"><h4><span class="domain-swatch" style="--domain-color:${escapeHtml(graphData.domains[domainId].color)}" aria-hidden="true"></span>${escapeHtml(graphData.domains[domainId].label)} <small>${nodes.length}</small></h4><ul>${nodes.map((node) => `<li><a href="/${conceptPath(node.id)}">${escapeHtml(stripInlineMath(node.label))}</a></li>`).join('')}</ul></section>`;
    }).join('');
...
export function renderStaticAtlasPage({
  ...
  const canonicalUrl = appUrl(atlasPagePath);
  const svgUrl = appUrl(atlasSvgPath);
  const pageTitle = `Complete ${graphData.meta.title}`;
...
<body>
  ...
  <main>
    ...
    <figure class="atlas-figure" aria-labelledby="complete-atlas-heading">
      ...
      <figcaption>This inline diagram is the exact output of the same SVG serializer used by the interactive atlas export command, with only the standalone XML declaration removed so it can be embedded in HTML. The unchanged standalone document remains available at <a href="${svgUrl}">${escapeHtml(svgUrl)}</a>.</figcaption>
    </figure>
  </main>
