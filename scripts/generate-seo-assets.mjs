...
function buildSitemapXml(graphData) {
  ...
  const urls = [appUrl(), ...fieldUrls, appUrl('concepts/'), ...concepts.map((node) => appUrl(conceptPath(node.id)))];
...
function buildLlmsTxt(graphData, graphDataPath, schemaPath) {
  ...
  return [
    ...
    '## Data', `- [Graph JSON](${appUrl(graphDataPath)})`, `- [JSON Schema](${appUrl(schemaPath)})`, `- [Concept Directory](${appUrl('concepts/')})`, '',
    '## Coverage', `- Fields: ${fields}`, `- Concepts: ${graphData.nodes.filter((node) => node.kind === 'structure').length}`, `- Nodes including construction junctions: ${graphData.nodes.length}`, `- Relations: ${graphData.edges.length}`, `- Domains: ${Object.keys(graphData.domains).length}`, '',
...
export async function generateSeoAssets({ graphData, distUrl, graphDataPath = 'data/atlas.json', schemaPath = 'data/schema.json' }) {
  await Promise.all([
    writeFile(new URL('robots.txt', distUrl), buildRobotsTxt()),
    writeFile(new URL('sitemap.xml', distUrl), buildSitemapXml(graphData)),
    writeFile(new URL('llms.txt', distUrl), buildLlmsTxt(graphData, graphDataPath, schemaPath))
  ]);
}
