...
function buildSitemapXml(graphData, viewsData, atlasSvgPath) {
  ...
  const urls = [appUrl(), appUrl(atlasSvgPath), ...fieldUrls, appUrl('concepts/'), appUrl('views/'), ...viewUrls, ...concepts.map((node) => appUrl(conceptPath(node.id)))];
...
function buildLlmsTxt(graphData, viewsData, graphDataPath, schemaPath, viewsPath, atlasSvgPath) {
  ...
  return [
    ...
    '## Data', `- [Graph JSON](${appUrl(graphDataPath)})`, `- [JSON Schema](${appUrl(schemaPath)})`, `- [Views JSON](${appUrl(viewsPath)})`, `- [All-in atlas SVG](${appUrl(atlasSvgPath)})`, `- [Concept Directory](${appUrl('concepts/')})`, `- [Guided Views](${appUrl('views/')})`, '',
...
export async function generateSeoAssets({ graphData, viewsData, distUrl, graphDataPath = 'data/atlas.json', schemaPath = 'data/schema.json', viewsPath = 'data/views.json', atlasSvgPath = 'static/atlas.svg' }) {
  await Promise.all([
    writeFile(new URL('robots.txt', distUrl), buildRobotsTxt()),
    writeFile(new URL('sitemap.xml', distUrl), buildSitemapXml(graphData, viewsData, atlasSvgPath)),
    writeFile(new URL('llms.txt', distUrl), buildLlmsTxt(graphData, viewsData, graphDataPath, schemaPath, viewsPath, atlasSvgPath))
  ]);
}
