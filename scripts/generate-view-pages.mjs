...
function renderViewPage(templateHtml, graphData, view) {
  ...
  html = html.replace(
    ...
    `<meta name="viewport" content="width=device-width, initial-scale=1">\n  <base href="../../">\n  <meta name="atlas:view" content="${escapeHtml(view.id)}">\n  <meta name="atlas:selection" content="node:${escapeHtml(view.nodeSequence[0])}">`
  );
...
function renderViewIndex(graphData, viewsData) {
  ...
  </style></head><body><main><header><a class="home" href="/">← Atlas home</a> · <a class="home" href="/static/atlas/">Complete static atlas</a><h1>Guided views</h1><p>Curated starting points for exploring the atlas. Each view applies a coherent set of fields, domains, relation types, and display settings, then starts a guided sequence of concepts. Use Previous and Next to follow the path, or explore freely without losing your place. Change any filter to leave the preset and continue independently.</p></header>${section('Featured paths', featured)}${other.length ? section('More views', other) : ''}</main></body></html>`;
}
